...
if ($result) {
    ...
    if ($password === $form_password) {
        ...
        if ($old_address === $new_address) {
            header("Location: dl_change_address.html?message=same_address");
        } else {
            $new_address = mysqli_real_escape_string($connection, $new_address);
            $update_query = "UPDATE dealer_registration_table SET address = '$new_address' WHERE id = $company_id";
            $update_result = mysqli_query($connection, $update_query);
            if ($update_result) {
                header("Location: dl_change_address.html?message=address_change_successfull");
            } else {
                echo "Company address update failed.";
            }
        }
    } else {
        header("Location: dl_change_address.html?message=wrong_password");
    }
} else {
